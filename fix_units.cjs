...
let jsContent = fs.readFileSync('temp_units.cjs', 'utf8');

// We need to parse this string into a JS array.
// The file starts with module.exports = [ ... ];
...
const jsonStr = JSON.stringify(units, null, 4);

// Double check if jsonStr is valid
try {
    JSON.parse(jsonStr);
} catch (e) {
    console.error("Stringify resulted in invalid JSON! " + e);
    process.exit(1);
}

const newTsContent = `import { UnitData } from '../types';\n\nexport const grammarUnits: UnitData[] = ${jsonStr};\n`;

fs.writeFileSync('src/data/unitsData.ts', newTsContent);
console.log("Successfully fixed unitsData.ts");
